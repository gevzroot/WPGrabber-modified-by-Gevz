...
*/
  class WPGHelper {
    ...
    // g
    public static  function selectProtocol() {

        global $wpdb;
        $stripID = $_GET['id'];
        $prot = $wpdb->get_row("SELECT * FROM wp_wpgrabber WHERE id = $stripID");

        switch ($prot->protocol){
            case 'http':
                $out  = '<select name="protocol">';
                $out .= '<option value="">Протокол не указан.</option>';
                $out .= '<option value="http" selected="selected">http</option>';
                $out .= '<option value="https">https</option>';
                $out .= '</select>';
                break;
            case 'https':
                $out  = '<select name="protocol">';
                $out .= '<option value="">Протокол не указан.</option>';
                $out .= '<option value="http">http</option>';
                $out .= '<option value="https" selected="selected">https</option>';
                $out .= '</select>';
                break;
            case '':
                $out  = '<select name="protocol">';
                $out .= '<option value="" selected="selected">Протокол не указан.</option>';
                $out .= '<option value="http">http</option>';
                $out .= '<option value="https">https</option>';
                $out .= '</select>';
                break;
