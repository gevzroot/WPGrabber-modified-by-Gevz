...
/*    function copyUrlFile($source, $dest)
    ...
    * @return string
    */
    function getUrl($url, $protocol) { //g
        if ($protocol == ''){$protocol = 'http';} //g
        ...
        if ($this->baseHrefs[$page]) {
          ...
          $page = $protocol.'://'.parse_url($page, PHP_URL_HOST);
    ...
    */
    function getImageUrl($url){
        ...
        if (substr_count($url, 'http://') or substr_count($url, 'https://')) {
            ...
            $page = 'http://' . parse_url($page, PHP_URL_HOST); //g
            $url = $page . '/' . ltrim($url, '/');
        }
        $url =  html_entity_decode($url);
    ...
    * 
    */
    private function _import($protocol)
    {
        ...
        if ($this->feed['type'] == 'html') {
            ...
            //g
            if ($protocol != ''){
                $this->_echo('Protocol: '.$protocol.'<br>');
                $this->_echo('CurrentURL: '.$this->currentUrl.'<br>');
            }
            ...
            if ($this->feed['params']['autoIntroOn'] == 1) {
                ...
                if ($this->feed['params']['orderLinkIntro']) { // порядок: анонс, ссылка
                    for ($k = 0; $k < count($matches); $k++) {
                        $this->introTexts[$this->getUrl($matches[$k][2], $protocol)] = $matches[$k][1]; //g
                    } 
                    $numArray = 2;          
                } else { // порядок: ссылка, анонс
                    for ($k = 0; $k < count($matches); $k++) {
                        $this->introTexts[$this->getUrl($matches[$k][1], $protocol)] = $matches[$k][2]; //g
                    }
                    $numArray = 1;          
                } 
            } else {
            ...
            foreach ($matches as $v) {
                $__url = $this->getUrl($v[$numArray], $protocol); //g
                $links[$__url] = $__url;
            }
        ...
        if ($this->feed['params']['filter_words_on']) {
            ...
            if ($this->feed['params']['filter_words_save']) { 
                if (count($_word_search[1])) {
                    $this->_echo('<br /><i>Материал <strong>НЕ</strong> будет сохранен по причине наличия следующих фильтр-слов в нем: ' . implode(', ', $_word_search[1]) . '</i>');
                    return null;
                } else {
                ...
                if (count($_word_search[1])) {
                    ...
                    $this->_echo('<br /><i>Материал <strong>НЕ</strong> будет сохранен по причине отсутствия фильтр-слов в нем' . '</i>');
                    return null;
                }
            } 
        }
...
*/        
    ...
    */
    final public function execute($id)
    {
        //g
        global $wpdb;
        $table_name = $wpdb->prefix . "wpgrabber";
        $proto = $wpdb->get_row("SELECT * FROM $table_name WHERE id = $id");
        ...
        $result = $this->_import($protocol); //g $protocol
